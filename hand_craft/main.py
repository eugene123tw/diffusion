...
from util import EmbedFC, ResidualConvBlock, UnetDown, UnetUp, plot_sample
...
class ContextUnet(nn.Module):
    def __init__(
        self, in_channels, n_feat=256, n_cfeat=10, pixel_height=28
        ...
        self.h = (
            pixel_height  # assume h == w. must be divisible by 4, so 28,24,20,16...
        )
...
model = ContextUnet(
    in_channels=3, n_feat=n_feat, n_cfeat=n_cfeat, pixel_height=pixel_height
).to(device)
...
def add_noise(samples, t, noise):
    return (
        alpha_bar.sqrt()[t, None, None, None] * samples
        + (1 - alpha_bar[t, None, None, None]).sqrt() * noise
    )
...
if TRAIN:
    ...
    for epoch in range(n_epoch):
        ...
        for i, (images, labels) in enumerate(data_loader):
            ...
            if i % 100 == 0:
                print(
                    f"Epoch [{epoch+1}/{n_epoch}], Step [{i}/{len(data_loader)}], Loss: {loss.item():.4f}"
                )
...
plot = plot_sample(
    ddpm_intermediate, n_sample, 4, save_dir, "ddpm_intermediate", None, save=True
)

plot = plot_sample(
    ddim_intermediate, n_sample, 4, save_dir, "ddim_intermediate", None, save=True
)
